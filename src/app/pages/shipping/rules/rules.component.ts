...
export class RulesComponent implements OnInit {
  ...
  ngOnInit() {
    if (localStorage.getItem('rulesCode')) {
      // setTimeout(() => {
      this.sharedService
        .getShippingRulesDetails(localStorage.getItem('rulesCode'))
        .subscribe(
          (rulesData) => {
            ...
            this.rules.endDate =
              rulesData.endDate && new Date(rulesData.endDate);
            this.rules.startDate =
              rulesData.startDate && new Date(rulesData.startDate);
  ...
  onChangeCode(e) {
    this.sharedService.checkCode(e.target.value).subscribe(
      (res) => {
        console.log(res);
        this.codeExits = true;
      },
  ...
  getRulesCriterias() {
    const fields = {};
    this.sharedService.getRulesCriterias().subscribe((data) => {
      // console.log(data)
      data.map((value) => {
        fields[value.code] = {
          name: value.name,
  ...
  getRulesActions() {
    this.sharedService.getRulesActions().subscribe((data) => {
      // console.log(data);
  ...
  onSubmit() {
    ...
    if (this.buttonText === 'Submit') {
      this.sharedService.createShippingRules(param).subscribe(
        (data) => {
          console.log(data);
      ...
      this.sharedService.updateShippingRules(this.rules.id, param).subscribe(
        (data) => {
          console.log(data);
          this.loadingList = false;
